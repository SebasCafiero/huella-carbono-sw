...
//        name = user.getName();
    ...
    public ModelAndView mostrarTodos(Request req, Response res) {
        ...
        parametros.put("trayectos", miembro.getTrayectos().stream().map(t -> { //No reutilizo mapeoTrayecto xq tramos y miembros tienen exceso de info
            Map<String, Object> trayectoMap = new HashMap<>();
            trayectoMap.put("id", t.getId());
            trayectoMap.put("mes", t.getPeriodo().getMes());
            trayectoMap.put("año", t.getPeriodo().getAnio());
            trayectoMap.put("tramos", t.getTramos().stream().map(tr -> tr.getMedioDeTransporte().getClasificacion()).collect(Collectors.toList())); //Al no agregar map, uso {{this}} para referenciar cada categoria
            trayectoMap.put("miembros", t.getMiembros().stream().map(m -> {
                Map<String, Object> miembroMap = new HashMap<>();
                miembroMap.put("nombre", m.getNombre());
                miembroMap.put("apellido", m.getApellido());
                return miembroMap;
    ...
    private List<Map<String, Object>> generarMapeoTransportes() {
...
//        transportesMap.put("publico", false);
    ...
    private Map<String, Object> mapeoTransporte(MedioDeTransporte transporte) {
        ...
        if(transporte instanceof TransportePublico) //todo ver de que solo aparezcan las paradas del transporte seleccionado o validar al atraparlo
            transporteMap.put("paradas", ((TransportePublico) transporte).getParadas().stream().map(p -> mapeoParada(p)).collect(Collectors.toList()));
...
//                coorFinal = trayecto.getTramos().get(cant).getUbicacionFinal().getCoordenada();
    ...
    public ModelAndView editar(Request req, Response res) {
        ...
        try {
            ...
            parametros.put("trayecto", mapeoTrayecto(trayecto));
            parametros.put("miembroID", idMiembro);
            parametros.put("miembros", trayecto.getMiembros().stream().map(this::mapeoMiembro).collect(Collectors.toList()));
            parametros.put("transportesTotales", generarMapeoTransportes());
            return new ModelAndView(parametros, "trayecto-edicion.hbs");
        } catch (NullPointerException e) {
            res.status(400);
